...
define(function(require, exports, module) {
	...
	var featureFactory = {
		syncScroll: function(state) {
			if (state === 'disabled') {
				return panelOption(
					'<span class="mirror__panel-option-comment">Sync scrolling is not available due to browser security restrictions :-(</span>'
				);
			}

			var ui = panelOption(
				'<input type="checkbox" name="sync-scroll" id="emr-sync-scroll" class="mirror__checkbox" />\
				<i class="mirror__icon" data-icon="checkbox"></i>\
				<label class="mirror__label" for="emr-sync-scroll">Sync scrolling</label>'
			);
		...
		overrideScroll: function(state) {
			var ui = panelOption(
				'<input type="checkbox" name="override-scroll" id="emr-override-scroll" class="mirror__checkbox" />\
				<i class="mirror__icon" data-icon="checkbox"></i>\
				<label class="mirror__label" for="emr-override-scroll">Override vertical scroll</label>'
			);
		...
		maxWidth: function(params) {
			...
			var ui = panelOption(
				...
				<i class="mirror__icon" data-icon="checkbox"></i>\
				<label class="mirror__label" for="emr-max-width-enabled">Max Viewport Width</label>\
				<input type="number" name="max-width" id="emr-max-width" value="" min="200" max="2000" size="4" maxlength="4" class="mirror__input" />'
			);
		...
		copyURL: function(handler) {
			var ui = panelOption('<button class="mirror__button" data-action="copy-url">Copy URL</button>');
		...
		reset: function(handler) {
			var ui = panelOption('<button class="mirror__button" data-action="reset">Reset</button>');
			var button = ui.querySelector('button');
			button.addEventListener('click', function(evt) {
				if (typeof handler === 'function') {
					handler();
	...
	function panelOption(innerHTML) {
		var elem = utils.el('div', 'mirror__panel-option');
		if (innerHTML) {
	...
	return {
		...
		create: function(features) {
			...
			var err = utils.el('p', 'mirror__error');
			...
			root.appendChild(utils.el('div', 'mirror__scroller'));
			if (features.panel) {
				var panel = utils.el('div', 'mirror__panel');
				var leftSection = utils.el('div', 'mirror__panel-section');
				var rightSection = utils.el('div', 'mirror__panel-section');
		...
		scroller: function(container) {
			...
			return container.querySelector('.mirror__scroller');
		},

		panel: function(container) {
			container = container || this.currentUI();
			return container.querySelector('.mirror__panel');
		},
