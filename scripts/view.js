define(function(require, exports, module) {
	...
	var dragDelegate = {
		onStart: function(data) {
			...
			view.classList.add('mirror__item_drag');
			dragData = {
				view: view,
			...
			utils.closest(view, 'mirror')
				.classList.add('mirror_drag');
		},
		...
		onEnd: function() {
			if (dragData && dragData.view) {
				utils.closest(dragData.view, 'mirror')
					.classList.remove('mirror_drag');;
			}
			dragData = null;
		}
	};
	...
	function addEvents(view) {
		var resizeHandler = view.querySelector('.mirror__item-ui-resize');
	...
	function findView(elem) {
		return utils.closest(elem, 'mirror__item');
	}
	...
	function createViewUI(view, bp) {
		var ui = utils.el('div', 'mirror__item-ui');
		ui.innerHTML = 
			'<div class="mirror__item-ui-top">\
				<i class="mirror__icon" data-icon="cross" data-action="close-view" title="Close view"></i>\
			</div>\
			<div class="mirror__item-ui-resize"></div>';

		var label = utils.el('div', 'mirror__item-ui-label');
		...
		view.appendChild(ui);
		view.appendChild(utils.el('div', 'mirror__item-ui-resize-overlay'));
	...
	return {
		...
		create: function(url, bp, options) {
			...
			var view = utils.el('div', 'mirror__item', {
				id: viewId
			});
			var iframe = utils.el('iframe', 'mirror-iframe', {
				src: url
			});
